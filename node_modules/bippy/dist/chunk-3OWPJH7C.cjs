...
${sampleLines[s].replace(" at new ", " at ")}`;
                const displayName = chunkNGVLXFEV_cjs.getDisplayName(fn);
  ...
  const name = fn ? chunkNGVLXFEV_cjs.getDisplayName(fn) : "";
...
var getFiberSource = async (fiber) => {
  ...
  const componentFunction = chunkNGVLXFEV_cjs.isHostFiber(fiber) ? chunkNGVLXFEV_cjs.getType(
    chunkNGVLXFEV_cjs.traverseFiber(
      fiber,
      (f) => {
        if (chunkNGVLXFEV_cjs.isCompositeFiber(f)) return true;
      },
      true
    )
  ) : chunkNGVLXFEV_cjs.getType(fiber.type);
  if (!componentFunction || reentry) {
    return null;
  }
  const frame = describeNativeComponentFrame(
    componentFunction,
    fiber.tag === chunkNGVLXFEV_cjs.ClassComponentTag,
